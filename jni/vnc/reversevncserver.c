...
static char KBD_DEVICE[256] = "/dev/input/event5";
...
void extractReverseHostPort(char *str)
{
  ...
  if (! rhost) {
    printf("reverse_connect: could not malloc string %d\n", len);
    exit(-1);
  }
...
static void init_fb(void)
{
	...
	fbmmap = mmap(NULL, 1920 * 1600, PROT_READ, MAP_SHARED, fbfd, 0);
...
init_input()
{
    ...
    if((inputfd = suinput_open("Privetvsem", &id)) == -1)
    {
      printf("cannot create virtual kbd device.\n");
    }
#endif
}
...
void injectTouchEvent(int down, int x, int y)
{
    ...
    {
#if 0
        suinput_write(inputfd, EV_REL, REL_X, 30);
        suinput_write(inputfd, EV_REL, REL_Y, 30);
